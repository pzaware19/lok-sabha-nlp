...
n_members    <- nrow(rs_ip)   # 264 matched via ideal-point pipeline (consistent with comparison page)
n_questions  <- 9315L         # total RS starred questions matched in N2 pipeline
...
The governing party pattern is striking and counterintuitive. BJP has the **lowest discipline** among all parties with at least 4 RS members (`r bjp_disc`), while `r most_disc$party_family` has the **highest** (`r round(most_disc$discipline, 3)`). The most disciplined parties -- YSRCP, TDP, DMK, AIADMK, BJD -- are all regional parties with narrow state-specific agendas.
